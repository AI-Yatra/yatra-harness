#!/usr/bin/env python3
"""harness-chat: a Claude Code-style REPL on top of the AI Yatra mini-harness.
...
Requires:
  - mini-harness synced with `uv sync` (openpyxl installed)
...
acceptance:
  ...
  timeout_seconds: 30
metadata:
  workshop_module: harness-chat
  difficulty: open-ended
  notes: |
    Generated by harness-chat REPL from a user message.
"""
    ...
    def run(self) -> int:
        print("harness-chat -- Claude Code-style REPL on the mini-harness")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Claude Code-style REPL for the mini-harness")
