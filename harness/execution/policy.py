...
def normalize_command(command: tuple[str, ...]) -> tuple[str, ...]:
    ...
    ``python3`` and ``python`` run the same interpreter, so a rule written for
    one has to bind the other. Mirrors ``tools._normalize_command``.
    """
...
def denied_pattern(
    ...
    A pattern matches as a contiguous subsequence anywhere in the command
    rather than only at the front, because the dangerous forms are reachable
    as arguments to a command that is legitimately allowed: ``python`` has to
    be runnable for the tests, and ``python -c "..."`` is arbitrary code. A
    prefix-only check is dodged by one inserted flag, and a rule that is
    trivially dodged is worse than none because it reads like a control.

    Module-level so the conversational REPL and ``harness run`` cannot drift
    on what counts as dangerous.
    """
