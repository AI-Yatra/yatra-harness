...
def _stop_group(process: subprocess.Popen, *, force: bool) -> None:
    ...
    The group can vanish between the timeout and the kill; that race is a
    success (the process is gone), not an error worth surfacing. SIGKILL only
    exists on POSIX, so Windows falls back to terminate/kill on the process.
    """
    try:
        if os.name == "posix":
