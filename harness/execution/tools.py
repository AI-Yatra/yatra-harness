...
def _git_apply(
    workspace: Workspace, patch: str, flags: tuple[str, ...], max_output: int
):
    """Run one `git apply` variant against the patch via a temp file.

    The patch goes through a file rather than stdin: subprocess with
    text=True on Windows translates \\n to \\r\\n on the way to the child,
    which `git apply` treats as a context mismatch against the LF file on
    disk. A file path preserves the bytes the model produced.
    """
    descriptor, name = tempfile.mkstemp(suffix=".diff")
    try:
        with os.fdopen(descriptor, "w", encoding="utf-8", newline="") as handle:
            handle.write(patch)
        return run_process(
            ["git", "apply", *flags, "--whitespace=nowarn", name],
            cwd=workspace.root,
            timeout=20,
...
def _apply_patch(
    ...
    check = _git_apply(workspace, patch, ("--check",), max_output)
    if check.returncode != 0:
        reverse = _git_apply(workspace, patch, ("--reverse", "--check"), max_output)
        ...
        three_way = _git_apply(workspace, patch, ("--check", "--3way"), max_output)
        applied_3way = None
        if three_way.returncode == 0:
            applied_3way = _git_apply(workspace, patch, ("--3way",), max_output)
        merged_cleanly = (
    ...
    applied = _git_apply(workspace, patch, (), max_output)
