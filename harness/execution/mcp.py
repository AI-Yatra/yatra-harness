...
class MCPStdioClient:
    ...
    def connect(self) -> None:
        ...
        # Bounded: only the most recent lines are useful in a diagnosis.
        self._stderr_tail: deque[str] = deque(maxlen=50)
        threading.Thread(target=self._read_stdout, daemon=True).start()
        # stderr has to be drained too. A server that logs enough to fill the
        # pipe buffer blocks on write, and then every request to it hangs.
    ...
    def request(self, method: str, params: dict[str, Any]) -> dict[str, Any]:
        ...
        while True:
            ...
            if message is None:
                ...
                raise MCPError(
                    f"MCP server exited while waiting for {method!r}{detail}"
                )
    ...
    def close(self) -> None:
        ...
        if process.stdin:
            process.stdin.close()
        # The reader threads exit on EOF, but the pipe objects are only
        # released when closed here; leaving them open leaks a file
        # descriptor per server for the life of the process.
