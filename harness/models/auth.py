...
2. **Take any key.** ``harness auth add <key>`` infers the provider when the
   key has a distinctive prefix; ``--provider`` resolves ambiguous shapes.
   Prefixes match longest-first, so ``sk-ant-api03-`` is not swallowed by
   ``sk-``.
...
PROVIDERS: tuple[Provider, ...] = (
    ...
    P("google", API_OPENAI, ("GOOGLE_API_KEY", "GEMINI_API_KEY"),
      # The /openai path is the Gemini API's OpenAI-compatible surface; the
      # bare /v1beta endpoint speaks a different wire format and rejects the
      # Bearer header this catalog's adapters and `auth verify` send.
      "https://generativelanguage.googleapis.com/v1beta/openai", ("AIza",),
      aliases=("gemini",)),
    P("opencode", API_OPENAI, ("OPENCODE_API_KEY",),
      "https://opencode.ai/zen/v1",
      aliases=("opencode-zen", "opencode-go", "zen"),
      note="OpenCode Zen; set the Go endpoint as a base URL when required"),
    P("commandcode", API_OPENAI, ("COMMAND_CODE_API_KEY",),
      "https://api.commandcode.ai/provider/v1", ("user_",),
      aliases=("command-code", "command_code", "cmd"),
      note="Command Code Provider API"),
...
def _stored_base_url(provider_name: str) -> str:
    """Return an operator-pinned endpoint without exposing its credential."""
    try:
        data = load_store()
    except AuthError:
        return ""
...
def verify(provider: str, timeout: float = 20.0) -> VerifyResult:
    ...
    base_url = _stored_base_url(resolved.name) or resolved.base_url
    if not base_url:
    ...
    try:
        ...
        with exc:  # the error holds the response socket until closed
